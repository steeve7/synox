  ...
  return (
    <div className="w-full lg:px-0 px-7">
      <div className="flex flex-col justify-center items-center mt-32 gap-4 text-center">
        <h2 className="bg-[#1c1c1e1a] text-[#1c1c1e] px-3 py-2 font-roboto font-medium uppercase rounded-md w-36 text-[15px]">
          Core features
        </h2>
        <p className="font-roboto font-bold md:text-[50px] text-[25px] xl:w-[50%] w-full text-[#1c1c1e]">
          5 Rapid Highlights of Our Synox Solution
        </p>
      </div>
      <div className="w-full flex flex-wrap gap-6 justify-center mt-10">
        {solutions.map(
          ({ image, title, desc, img, top, bottom, BgColor }, i) => (
            <div
              key={i}
              className={`flex flex-col ${
                i < 2
                  ? "w-full md:w-[40%]"
                  : " w-full lg:w-[26%] md:w-[40%]"
              } ${BgColor} rounded-3xl gap-4 px-10 py-10 min-h-[300px] ${i === solutions.length - 1 ? "lg:mr-0 md:mr-[300px] mr-0" : "" }`}
            >
              <span className="w-16 p-4 bg-white rounded-full shadow-lg flex justify-center items-center">
                {image}
              </span>
              <h2 className="font-roboto font-bold lg:text-[30px] md:text-[20px] text-[20px] text-[#1c1c1e]">
                {title}
              </h2>
              <p className="font-roboto font-medium md:text-[18px] text-[15px] text-[#656565]">
                {desc}
              </p>
              <img
                src={img}
